...
const SearchProblem = () => {
  ...
  const debouncedSearch = useCallback(
    debounce(async (query: string) => {
      ...
      try {
        const response = await fetch(`http://localhost:3002/api/search/searchProblems?query=${query}`, {
          method: "GET",
        });
        if (!response.ok) {
          ...
          {searchQuery && searchResults.length > 0 && (
            <ul className="space-y-4">
              {searchResults.map((result: any, index: number) => (
                <li
                  key={index}
                  className="bg-gray-50 p-4 rounded-lg shadow-md hover:bg-gray-100 transition hover:transition-transform translate-y-1"
